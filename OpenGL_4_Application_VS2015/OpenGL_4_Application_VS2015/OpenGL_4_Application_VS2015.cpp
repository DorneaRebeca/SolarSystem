...
void followShip() {
	...
	float offsetX = sin(shipAngle);
	float offsetZ = cos(shipAngle);

	if (shipAngle >= glm::radians(-90.0f) && shipAngle < glm::radians(0.0f)) {
		xoffset = (glm::radians(-90.0f) - shipAngle)*ymove;
		yoffset = shipAngle*ymove;
	}
	if (shipAngle < glm::radians(-90.0f) && shipAngle >= glm::radians(-180.0f)) {
		xoffset = (glm::radians(-90.0f) - shipAngle)*ymove;
		yoffset = (glm::radians(-180.0f) - shipAngle)*ymove;
	}
	if (shipAngle >= glm::radians(90.0f) && shipAngle < glm::radians(180.0f)) {
		xoffset = (shipAngle - glm::radians(90.0f))*ymove;
		yoffset = (glm::radians(180.0f) - shipAngle)*ymove;
	}
	if (shipAngle >= glm::radians(0.0f) && shipAngle < glm::radians(90.0f)) {
		xoffset = (glm::radians(90.0f) - shipAngle)*(-ymove);
		yoffset = shipAngle*(ymove);
	}
	...
	newPosition.x -= xoffset - 0.2f ;
	newPosition.y += 0.4f;
	newPosition.z += yoffset + 1.5f;
	myCamera.set(newPosition);
	myCamera.move(gps::MOVE_RIGHT, -xoffset*50);

	float newYaw = shipAngle*180/PI;


	myCamera.rotate(pitch, -90 - (90 + shipAngle * 180 / PI));
...
void processMovement()
{
	...
	if (pressedKeys[GLFW_KEY_T]) {
		//myCamera.set(initialShip + glm::vec3(0.2f, 0.4f, 1.5f), glm::vec3(0.0f, 0.0f, 0.0f));
	...
	//compute movement for ship
	if (shipAngle >= glm::radians(-90.0f) && shipAngle < glm::radians(0.0f)) {
		xactual = (glm::radians(-90.0f) - shipAngle)*ymove;
		yactual = shipAngle*ymove;
	}
	if (shipAngle < glm::radians(-90.0f) && shipAngle >= glm::radians(-180.0f)) {
		xactual = (glm::radians(-90.0f) - shipAngle)*ymove;
		yactual = (glm::radians(-180.0f) - shipAngle)*ymove;
	}
	if (shipAngle >= glm::radians(90.0f) && shipAngle < glm::radians(180.0f)) {
		xactual = (shipAngle - glm::radians(90.0f))*ymove;
		yactual = (glm::radians(180.0f) - shipAngle)*ymove;
	}
	if (shipAngle >= glm::radians(0.0f) && shipAngle < glm::radians(90.0f)) {
		xactual = (glm::radians(90.0f) - shipAngle)*(-ymove);
	...
	shipAngle += glm::radians(-xmove*10);
	if ((shipAngle + glm::radians(-xmove*10)) < glm::radians(-180.0f))
	{
...
int checkForCollision(glm::vec3 obj1, glm::vec3 obj2) {
	if((obj1.x - obj2.x)*(obj1.x - obj2.x) + (obj1.z - obj2.z)*(obj1.z - obj2.z)<5)
		return true;
	return false;
}
...
void renderScene()
{
	...
	model = glm::translate(model, initialShip);
